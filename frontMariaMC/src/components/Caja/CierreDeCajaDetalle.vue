<template>
    <v-sheet class="mx-auto">
        <v-row align="center" justify="center">
            <h3 class="classMaria mt-1">Cierre de Caja Detalle</h3>
        </v-row>
        <v-row >
            <v-btn class="mx-3" color="primary" prepend-icon="mdi-arrow-left-thick" variant="elevated" 
                        @click.prevent="volver">Volver</v-btn>
        </v-row>
        <div class="mt-3">
            <v-row align="center" justify="center">
                    <div ><b>Fec. Desde: </b> {{ fechaDesde }}</div>
            ...
            <v-row>
                <v-col>
                    <v-data-table id="tablaCierre" :headers="headers" :fixed-footer="true" :items="ListaItems"
                        :dense="true" :items-per-page="20" :sort-by="[{ key: 'idMovimiento', order: 'asc' }]"
...
onMounted(()=>{
    if (route.params.idCierre != null && route.params.idCierre != undefined && route.params.idCierre != '') {

        let idCierreAux = 0
        try {
            idCierreAux = parseInt(route.params.idCierre.toString())
            CargarCierre(idCierreAux)
        }
        catch (e) {
            modalRef.value.ModalErrorShow("Cierre de caja Detalle ",  "Ocurrío un error al cargar el detalle del cierre de caja.", false)
...
const CargarCierre = async (idCierre:number) => {
    //procesar Venta
    try {
        leyendaTabla.value = "Cierre de Caja Detalle"
        procesando.value = true
        modalLoaderRef.value.Show()

        let response = await storeCierre.GetCierreDeCaja(idCierre)

        modalLoaderRef.value.Close()
        procesando.value = false

        if (response.result == "ok") {
            cargarDatos(response.cierre)
        } else {
            modalRef.value.ModalErrorShow("Cierre de caja Detalle ", response.message, false)
        }
        ...
        if (modalLoaderRef.value)
            modalLoaderRef.value.Close()
        procesando.value = false
        modalRef.value.ModalErrorShow("Cierre de caja Detalle", error, false)
    }
